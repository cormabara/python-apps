...
from config import WorkMode, MbConfig
from mb_types import S32
from numpy import int32

from mb_ranges import saturate_out_of_range
from mb_logger import Logger
from mb_tools import div_sqr3, divshx

TRIGO_THETA_RANGE_RAD = 2 * math.pi
...
TRIGO_THETA_MAX = TRIGO_THETA_RANGE - 1  # Questo il range reale del theta
TRIGO_THETA_MIN = -TRIGO_THETA_MAX  # Questo il range reale del theta
TRIGO_THETA_WRAP = TRIGO_THETA_MAX

TRIGO_OUT_SHIFT = 14
TRIGO_OUT_RANGE = (2 ** TRIGO_OUT_SHIFT)         # Range di uscita della funzione trigo 0..16383 quindi 16384 elementi
TRIGO_OUT_WRAP  = TRIGO_OUT_RANGE-1
TRIGO_OUT_MIN = -TRIGO_OUT_WRAP
TRIGO_OUT_MAX = TRIGO_OUT_WRAP
pix2e16 = 205887

THETA_RAD_TO_CUSTOM = TRIGO_THETA_RANGE / TRIGO_THETA_RANGE_RAD
THETA_CUSTOM_TO_RAD = TRIGO_THETA_RANGE_RAD / TRIGO_THETA_RANGE
...
def _getSinTable(deep_):
    ...
    in_v = np.round(TRIGO_OUT_MAX * np.sin(in_s), 0)
    return in_v
...
def _getCosTable(deep_):
    ...
    step = (math.pi / 2) / deep_
    in_s = numpy.arange(0, (math.pi / 2) + step, step)
    in_v = np.round((2 ** TRIGO_OUT_SHIFT) * np.cos(in_s), 0)
    return in_v
...
TabSin_Fpga = [ 0,   101,   201,   302,   402,   503,   603,   703,   804,   904,
                1005,  1105,  1205,  1305,  1406,  1506,  1606, 1706,  1806,  1906,
                2005,  2105,  2205,  2304,  2404,  2503,  2603,  2702,  2801,  2900,
                2999,  3098,  3196,  3295, 3393,  3491,  3590,  3688,	 3785,  3883,
                3981,  4078,  4175,  4273,  4370,  4466,  4563,  4659,  4756,  4852,
                4948,  5044,  5139,  5234,  5330,  5425,	5519,  5614,  5708,  5802,
                5896,  5990,  6083,  6177,  6270,  6362,  6455,  6547, 6639,  6731,
                6822,  6914,  7005,  7095,  7186,  7276,  7366,  7456,  7545,  7634,
                7723,  7811,  7900,  7988,  8075,  8162,  8249,  8336,  8423,  8509,
                ...
                11002, 11076, 11150, 11224, 11297, 11369, 11441, 11513, 11585, 11655,
                11726, 11796, 11865, 11934, 12003, 12071, 12139, 12206, 12273, 12339,
                12405, 12471, 12536, 12600, 12664, 12728, 12791, 12853, 12915, 12977,
                13038, 13099, 13159, 13219, 13278, 13336, 13394, 13452, 13509, 13566,
                13622, 13678, 13733, 13787, 13841, 13895, 13948, 14000, 14052, 14104,
                14154, 14205, 14255, 14304, 14353, 14401, 14449, 14496, 14542, 14588,
                14634, 14679, 14723, 14767, 14810, 14853, 14895, 14936, 14977, 15018,
                15058, 15097, 15136, 15174, 15212, 15249, 15285, 15321, 15356, 15391,
                15425, 15459, 15492, 15524, 15556, 15587, 15618, 15648, 15678, 15706,
                15735, 15762, 15790, 15816, 15842, 15867, 15892, 15916, 15940, 15963,
                15985, 16007, 16028, 16048, 16068, 16088, 16106, 16124, 16142, 16159,
                16175, 16191, 16206, 16220, 16234, 16247, 16260, 16272, 16283, 16294,
                16304, 16314, 16323, 16331, 16339, 16346, 16352, 16358, 16363, 16368,
                16372, 16375, 16378, 16380,	16382, 16383]
...
def _sinAq(theta_: int):
    """ "teta_" puo' avere un valore qualsiasi, positivo o negativo;
        ...
        ATTENZIONE: l'uscita è sempre un int32 per simulare correttamente dice """
...
def trigo_sinAq(theta_):
    if MbConfig().work_mode == WorkMode.theo:
        return np.sin(theta_)
    else:
        return _sinAq(theta_)


def trigo_cosAq(theta_):
    if MbConfig().work_mode == WorkMode.theo:
...
def trigo_ksin(factor_,theta_):
    return factor_ * _sinAq(theta_)


def trigo_kcos(factor_, theta_):
        return factor_ * _cosAq(theta_)
