...
class PhasesPll:
    ...
    def stimulus(self, amplitude_, frequency_):
        self.step_custom = TRIGO_THETA_RANGE * frequency_ / self.sample_frequency_hz
    ...
    def _loop_ApplyPark(self, rm_):
        """"""
        if (rm_):
            self.theta_park = self.prev_theta_out % self.THETA_CUSTOM_RANGE
            # self.theta_park = self.theta_in_custom
        else:
            self.theta_park = (self.prev_theta_out / self.THETA_RAD_TO_CUSTOM) % (2 * math.pi)
    ...
    def _loop_CalculateOutputs(self):
        ...
        self.omega_out = tmp_omega / (2 ** self.NEW_SHIFT)

        self.real_teta_out = tmp_teta
        self.real_omega_out = tmp_omega
        self.theta_out_rad = (self.theta_out_custom / self.THETA_RAD_TO_CUSTOM) % (2 * math.pi)
    ...
    def _loop_UpdateVectors(self, sample_):
        ...
        self.omega_out_v.append(self.omega_out)
    ...
    def CalculateLoop(self, real_, amplitude_, frequency_):
        self.stimulus(amplitude_,frequency_)
        ...
        for sample in self.display_range:
            self.Calculate(real_, theta_in)
            self._loop_UpdateVectors(sample)
            theta_in += self.step_custom
